...
const { buildKeyIndex } = require('./secrets-bws-helpers');

// Lazy-loaded to avoid pulling in @bitwarden/sdk-napi at require time.
// This keeps parseInjections unit-testable without the native SDK installed.
function _bwsClient()   { return require('./secrets-bws-client'); }
function _bwsCommands() { return require('./secrets-bws-commands'); }
...
async function fetchSecretsForKeys(keys) {
  const { createClient, getOrgId } = _bwsClient();
  const { fetchAllSecrets }        = _bwsCommands();
