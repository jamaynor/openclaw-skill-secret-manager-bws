...
describe('secrets-bws-mcp-wrapper integration', { skip: SKIP }, () => {
  ...
  it('injects secret as environment variable', () => {
    const out = wrapper([
      '--secret', WRAPPER_KEY, '--env', 'INJECTED_VAR',
  ...
  it('injects secret as a CLI argument', () => {
    ...
    const out = wrapper([
      '--secret', WRAPPER_KEY, '--arg', 'INJECTED_MARKER',
      '--', 'node', '-e', "const i=process.argv.indexOf('INJECTED_MARKER'); process.stdout.write(process.argv[i+1])",
    ]);
    assert.equal(out, 'wrapper-test-value');
  });
